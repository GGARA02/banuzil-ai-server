...
- **1단계 무한 정체 방지(강제 게이트)**: 양측 2신호 조건은 철회형(회피) 파트너가 신호를 잘 안 내면 충족되지 않아 1단계에 갇힐 수 있음. `CYCLE_FORCE_ROUNDS`(4) — 1단계 누적 4R 이상이면 신호 미달이어도 사이클 절차로 강제 진입. (2→3 강제 게이트와 동일 패턴)
