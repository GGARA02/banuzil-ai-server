...
- 응답 길이를 라운드마다 바꿔라. 어떤 라운드는 2~3문단으로, 어떤 라운드는 2~4문장으로 짧고 강하게 끝내라. 매번 같은 길이로 길게 늘이지 마라.
...
- 매 라운드 상대 입장("그분은 ~")부터 시작하지 마라. 내담자가 방금 꺼낸 감정을 먼저 받아 충분히 머무는 라운드도 두고, 상대의 마음 전달은 그 뒤에 잇거나 짧게 한 줄로 줄여도 된다(상대 발화 전달 자체는 유지하되, 매 라운드 길게 첫머리에 둘 필요는 없다). 응답의 시작점(내담자 감정 / 상대 마음 / 반영 / 질문)을 라운드마다 바꿔라.
- 매 라운드 똑같은 "상대방 요약 → 질문" 골격을 반복하지 마라. EFT의 여러 개입 — 반영(Reflection), 타당화(Validation), 환기적 반응(Evocative Responding), 심화(Heightening), 공감적 추론(Empathic Conjecture) — 중 그 라운드의 정서 흐름에 맞는 방식을 골라 다양하게 전개하라. 다만 대부분의 라운드는 내담자가 답할 수 있는 한 가지(열린 질문이나 구체적 초대)로 마무리해 대화가 이어지게 하라 — 내담자는 물어봐야 답한다. 깊은 반영·타당화로 끝내는 라운드도 가능하지만, 그럴 때조차 내담자가 이어 말할 여지를 남겨라. 단, '요약→질문'의 똑같은 형태로 매번 굳히지는 마라.
- "혹시 ~한 마음이 아니었을까요 / ~수도 있겠어요" 같은 추론형 표현은 강력하지만, 매 응답·매 문단 반복하면 공식처럼 들린다. 같은 공감적 추론이라도 반영·타당화·심화의 형태로 바꿔 말하고, 이 표현을 한 응답에서 한 번 이하로만 쓰도록 절제하라.
...
② 상대방의 표면 행동(비난·침묵·날카로운 말) 이면의 1차 감정과 애착 욕구는 EFT 공감적 추론(Empathic Conjecture)으로 따뜻하게 짚어줄 수 있다. 단정형이 아니라 열린 어조로 제시하되("혹시 ~한 마음이 아니었을까요" 외에도 "~처럼 들려요", "~게 느껴지지 않았을까 싶어요" 등 표현을 바꿔가며), 같은 추론 표현을 매 라운드 반복하지 마라. 양측 모두를 애착 욕구의 관점으로 바라보라(어느 한쪽도 악인으로 묘사 금지). 확정적 단정("~였을 거예요")이나 옳고 그름 평가는 하지 마라.
...
def build_user_message(
    ...
    prohibit_note = (
        ...
        "② 상대방의 1차 감정·애착 욕구는 열린 어조로 따뜻하게 짚을 수 있다(\"혹시 ~한 마음이 아니었을까요\" 외에 \"~처럼 들려요\" 등으로 표현을 바꿔가며, 매 라운드 같은 표현을 반복하지 마라). "
    ...
    if bullet_detected:
        content_rule = (
            ...
            "③ 이번 라운드는 진정·안정(de-escalation)이 우선이다. 깊이 파고드는 탐색 질문은 삼가되, "
            "마지막은 내담자가 부담 없이 답할 수 있는 부드러운 한마디(예: '지금 마음은 좀 어때요?')로 닫아 "
            "대화가 끊기지 않게 하라.\n"
        ...
        content_rule = (
            ...
            "① 시작점을 매 라운드 바꿔라. 어떤 라운드는 내담자가 방금 꺼낸 감정을 먼저 받아 머물고, "
            f"어떤 라운드는 상대방 마음 전달이나 반영으로 시작해도 된다. '상대방({other_label})은 ~'으로 매번 시작하지 마라.\n"
            f"② 상대방({other_label})의 실제 발화는 이번 라운드 흐름에 맞게 전달하되, 단순 사실 나열이 아니라 "
            "표면 행동(비난·침묵·날카로운 말) 이면의 1차 감정·애착 욕구로 따뜻하게 재구성하라. "
            "매 라운드 길고 동일한 위치에 둘 필요는 없다 — 짧게 한 줄로 잇는 라운드도 둬라. "
            "단, 상대방이 하지 않은 말이나 행동을 지어내지 마라.\n"
            "③ 아래 EFT 개입 중 이번 라운드 정서 흐름에 맞는 1~2가지를 골라 전개하라. "
            ...
            "④ 가능하면 내담자가 답할 수 있는 한 가지(열린 질문이나 구체적 초대)로 마무리해 대화가 이어지게 하라 "
            "— 내담자는 물어봐야 답한다. 깊은 반영·타당화로 끝내도 좋지만 그럴 때도 이어 말할 여지를 남기고, "
            "'요약→질문' 형태를 매번 똑같이 반복하지는 마라.\n"
            "⑤ '혹시 ~한 마음이 아니었을까요' 같은 추론 표현을 이번 응답에서 한 번을 넘겨 반복하지 마라. "
            "같은 추론도 반영·타당화·심화로 바꿔 말하라.\n"
            "⑥ 응답 길이를 라운드마다 바꿔라. 어떤 라운드는 2~4문장으로 짧고 또렷하게 끝내라.\n"
            "⑦ 패턴 분석, 사이클 설명, 애착 이론을 상담 응답에 직접 언급하지 마라.\n"
            "⑧ 상대방이 침묵했다, 물러났다 등 실제 발화에 없는 행동을 만들어내지 마라."
        )
...
def build_cycle_definition_prompt(
    ...
    return (
        ...
        "두 사람이 반복해서 빠지는 부정적 상호작용 사이클(The Cycle)을 분석하여 간결하고 명확하게 정의하라.\n\n"
        "핵심 — 무엇을 그릴 것인가:\n"
        "- 이번에 있었던 일회성 사건(예: 특정 여행 일정)을 매번 반복되는 일처럼 쓰지 마라. "
        "그 사건은 사이클이 시작되는 '계기'로만 도입부에 짧게 언급하고, 그 자체를 반복 고리의 한 칸으로 넣지 마라.\n"
        "- 진짜 반복되는 것은 '한쪽의 감정·행동이 다른 쪽의 감정·행동을 부르고, 그 반응이 다시 처음 반응을 키우는' 마음의 악순환이다. "
        "이 추격-위축(다가감-물러남)의 감정 춤을 고리의 중심에 놓아라.\n"
        "- 고리가 한 바퀴 돌아 다시 처음 반응으로 이어지도록 닫아라(그래야 '반복됩니다'가 자연스럽다).\n\n"
        "형식:\n"
        "- 3~5문장으로 작성하라.\n"
        "- 예: '한 사람이 ~한 마음에 ~하면, 다른 사람은 ~하게 느껴 ~하고, 그러면 처음 사람은 더욱 ~하게 되는 패턴이 반복됩니다.'\n"
        "- 여성은 '여자친구', 남성은 '남자친구'로 지칭하라.\n"
        "- 전문 용어 없이 이해하기 쉬운 언어로 작성하라.\n\n"
        "어조:\n"
        "- 어느 한쪽도 탓하지 말고, 두 사람 모두 이 패턴에 함께 갇혀 힘들어하는 시선으로 따뜻하게 서술하라.\n"
        "  (이는 어조에 대한 지시이지, 결과에 넣을 문장이 아니다.)\n"
        "- '~해버리다'처럼 한쪽 행동을 탓하는 뉘앙스의 표현을 피하고, 행동을 중립적으로 묘사하라.\n\n"
        "금지:\n"
        "- '두 사람 모두 피해자임을 강조합니다 / 강조할 수 있습니다' 같은 메타·결론·요약 문장을 끝에 붙이지 마라.\n"
        "- 사이클(패턴)을 서술하는 문장 자체로 끝내라. 분석·설명·질문을 덧붙이지 마라.\n"
        "- 사이클 정의 본문만 출력하라.\n\n"
        f"[여성 상담 기록]\n{f_history_text}\n\n"
        f"[남성 상담 기록]\n{m_history_text}"
        f"{answer_block}"
...
# ── 사이클 정의 직후 상담사 브릿지 메시지 ─────────────────
def build_cycle_bridge_message_prompt(is_female: bool, cycle_definition: str) -> str:
    """
    사이클 정의가 완료된 직후, 내담자에게 전달할 상담사 메시지를 만든다.
    EFT Step 4(문제 재구성)에 해당 — 함께 만든 사이클을 따뜻하게 비춰주고,
    그 이면의 1차 감정·애착 욕구로 시선을 옮기며 2단계(더 깊은 마음 표현)로 부드럽게 잇는다.
    build_system_prompt와 짝으로 call_llm_with_history의 user 메시지로 사용.
    """
    self_label  = "여성" if is_female else "남성"
    other_label = "남성" if is_female else "여성"

    return (
        f"두 분과 함께 1단계 상담을 진행하며, 두 사람이 반복해 온 상호작용 패턴(사이클)을 아래와 같이 정리했어요.\n\n"
        f"[정리된 사이클 — 내부 참고용]\n{cycle_definition}\n\n"
        f"지금은 {self_label}(당신)에게 이 사이클을 따뜻하게 비춰주고, 다음 단계로 넘어가는 길을 여는 상담사 메시지를 작성하라.\n"
        f"[작성 지침 — EFT 문제 재구성 + 2단계 전환]\n"
        f"① 위 사이클의 핵심을, 어느 한쪽의 잘못이 아니라 '두 사람이 함께 빠지는 반복'으로 당신에게 부드럽게 비춰주라. "
        f"이때 '사이클', '패턴', '애착' 같은 이론·분석 용어를 그대로 쓰지 말고, 두 분이 겪은 실제 상황의 언어로 풀어서 말하라.\n"
        f"② 그 반복 안에서 당신과 {other_label} 각자가 느꼈을 표면 아래 1차 감정(두려움·외로움·서운함 등)과 "
        f"진짜 바라던 마음(연결·인정·안심)을 공감적 추론형(\"혹시 ~한 마음이 아니었을까요\")으로 따뜻하게 짚어라. "
        f"양측 모두를 애착 욕구의 관점으로 바라보고, 어느 쪽도 악인으로 묘사하지 마라.\n"
        f"③ 이제는 그 반복에 갇히는 대신, 서로에게 정말 하고 싶었던 속마음을 한 걸음 더 꺼내볼 수 있도록 부드럽게 초대하며 마무리하라. "
        f"무겁지 않게, 당신이 다음에 이야기할 거리가 자연스럽게 생기도록 열어두어라.\n\n"
        f"[출력 규칙 — 반드시 준수]\n"
        f"① 도입 안내 문장(\"말씀드릴게요\" 등) 없이 곧바로 시작하라.\n"
        f"② 2~3문단 이내로 따뜻하고 간결하게. 당신이라는 호칭만 사용하고, 이름·애칭·'자기'는 쓰지 마라.\n"
        f"③ {other_label}의 마음을 전할 때 '상대분/그분'으로 지칭하고, 단정형(\"~였을 거예요\")이나 옳고 그름 평가는 하지 마라.\n"
        f"④ 실제 발화에 없는 행동을 지어내지 마라.\n"
        f"⑤ 사이클 정의 원문이나 위 지침 텍스트를 그대로 인용·복붙하지 마라. 상담사 자신의 말로 재구성하라."
    )
...
mediation_plans (중재안):
- 중재안 3개. 각 중재안은 아래 형식을 정확히 따른다(다른 형식·마크다운 금지):
  1. {{중재안 제목}}
  필요한 이유: {{2~3문장}}
  실천 포인트
  - {{포인트}}
  - {{포인트}}
  - {{포인트}}
- 중재안 사이는 빈 줄 하나로 구분한다. 번호는 "1." "2." "3.", 목록은 "- "로 통일한다.

recommended_dialogues (추천 대화법):
- 중재안 3개와 같은 순서·같은 제목으로, 각 중재안 아래에 예시 문장 3~4개. 아래 형식을 정확히 따른다:
  1. {{중재안 제목}}
  "{{내담자가 해볼 수 있는 예시 문장}}"
  "{{예시 문장}}"
  "{{예시 문장}}"
- 예시 문장은 반드시 큰따옴표로 감싼 완성된 한 문장으로 쓴다. "당신:", "나:" 같은 화자 접두사를 붙이지 마라.
- 중재안 사이는 빈 줄 하나로 구분한다.
- 모든 중재안 예시 뒤에, 빈 줄 하나를 두고 마무리 2~3문장(이번 상담의 가장 중요한 변화/핵심을 따뜻하게 정리)을 덧붙인다.

[형식 일관성 — 반드시 준수]
- 위 형식(번호 "1./2./3.", 목록 "- ", 예시는 큰따옴표 문장, 섹션 내 항목은 빈 줄로 구분)을 그대로 지켜라.
- 항목을 한 줄에 이어 쓰지 말고 각 항목·예시를 줄바꿈으로 분리하라.
