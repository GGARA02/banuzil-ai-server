    ...
    if eft_stage == 1:
        ...
        if stage3_round < 3:
            ...
            goal = (
                ...
                "스스로 도출하도록 도우십시오. 누가·언제·어떻게 할지 손에 잡히는 약속으로 "
                "구체화하되, AI가 답을 정해주지 말고 두 사람이 직접 합의하도록 촉진하십시오. "
                "아직 종결을 서두르지 말고, 합의가 현실적으로 자리 잡도록 함께 다듬으십시오."
            )
        else:
            # Step 9: 안전 애착 공고화 (변화 서사 정리 + 재발 예방)
            base = "EFT 3단계 Consolidation — Step 9 [안전 애착 공고화]:"
            goal = (
                "이제 합의를 넘어, 두 사람이 이번 상담에서 함께 만들어낸 변화의 이야기를 "
                "정리하고 확인하도록 도우십시오. '예전에는 한쪽이 다가갈수록 다른 쪽이 멀어지는 "
                "사이클에 갇혀 있었는데, 이제는 ~하게 되었다'처럼 두 사람이 함께 빠져나온 과정을 "
                "비춰주십시오. 그리고 그 사이클이 다시 찾아올 때 서로 알아차리고 손 내밀 수 있다는 "
                "점을 강화하여 재발을 예방하십시오. 새로 만든 안전한 유대를 두 사람의 언어로 "
                "확인하며 따뜻하게 마무리해 가십시오."
            )
