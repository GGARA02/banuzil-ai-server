...
def build_stage_instruction(
    ...
    stage3_round: int = 0,  # 3단계 누적 라운드 (Step 8→9 분화용)
) -> str:
    """
    ...
    if eft_stage == 1:
        ...
        # 라운드 기반 상한 (라운드1만 동맹 형성에 고정, 이후엔 신호에 따라 진전)
        # 과거엔 라운드<=3까지 강하게 캡을 걸어 1단계에 과도하게 머물렀음 → 완화.
        if round_num <= 1:
            round_cap = 1
        else:
            round_cap = 3
        ...
        # 라운드 1: 공감과 경청에 집중, 신호 압박 없음 (이후엔 자연스러운 신호 유도 시작)
        if round_num <= 1:
            goal = (
        ...
        if stage3_round < 3:
            # Step 8: 새로운 해결책 촉진 (일상 적용 합의)
            base = "EFT 3단계 Consolidation — Step 8 [새로운 해결책 촉진]:"
            goal = (
                "두 사람이 변화된 소통 방식을 일상의 구체적 상황에 적용하는 합의안을 "
                "스스로 도출하도록 도우십시오. AI가 답을 정해주지 말고, 내담자가 직접 답할 수 있는 "
                "구체적인 질문 하나로 마무리하십시오(예: '어떤 상황에서, 어떤 한마디면 당신에게 충분할까요?'). "
                "긴 설명이나 전체 요약 대신, 짧게 짚고 내담자가 이어 말하도록 여십시오. 종결을 서두르지 마십시오."
            )
        else:
            # Step 9: 안전 애착 공고화 (변화 서사 정리 + 재발 예방)
            base = "EFT 3단계 Consolidation — Step 9 [안전 애착 공고화]:"
            goal = (
                "공고화 단계지만, 매 라운드 전체 변화 서사를 통째로 다시 요약하지 마십시오(긴 모놀로그 금지). "
                "대신 내담자가 직접 답할 수 있는 질문 하나를 던져, 변화를 내담자 스스로의 말로 꺼내게 하십시오 "
                "(예: '오늘 이야기 중 가장 마음에 남는 한 가지는 뭐예요?', '다음에 그 순간이 또 오면, 자신에게 "
                "혹은 그분에게 어떤 한마디를 건네보고 싶어요?'). 상담사가 다 정리해 주기보다, 내담자의 대답을 "
                "이끄는 짧은 마무리로 끝내십시오. 재발 예방도 '다시 그 순간이 오면 무엇을 먼저 알아차리고 싶은지'를 "
                "내담자가 답하도록 묻는 방식으로 다루십시오."
            )
...
▶ 1단계→2단계 전진: 사이클 정의 완료 여부에 따라 코드가 직접 처리한다. stage를 2로 올리지 마라 (1로 유지).
...
▶ 3단계 종료: progress는 3단계에서 라운드가 진행될수록 점진적으로 올려라(매 라운드 약 +25~30).
   양측의 새로운 합의·유대 정리가 충분히 이뤄졌을 때 90 이상으로 올려 종료 신호를 보내라. readyForEnd는 항상 false.
